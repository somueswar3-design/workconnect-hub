...
const ClientOverview = () => {
  ...
  const loadData = async () => {
    setIsLoading(true);
    try {
      const [freelancersData, statsData] = await Promise.all([getHiredFreelancers(), getClientStats()]);
            ...
            <div className="flex items-center justify-between mb-3">
              <TabsList className="h-8">
                <TabsTrigger value="active" className="text-xs px-3 gap-1.5">
                  <Users className="h-3.5 w-3.5" />Hired Freelancers
                ...
                <TabsTrigger value="history" className="text-xs px-3 gap-1.5">
                  <History className="h-3.5 w-3.5" />Project History
